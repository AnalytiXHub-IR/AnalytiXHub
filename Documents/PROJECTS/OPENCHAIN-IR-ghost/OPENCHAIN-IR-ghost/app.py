...
def index():
    ...
    # Cases are already sorted by DB query
...
def investigation(case_id):
    ...
    # Reload case context
    db = case_manager.get_db()
    case = db.query(DBCase).filter_by(id=case_id).first() # Assuming case_id maps to 'id' in DBCase
    if case:
        session['active_case_id'] = case_id
        session['active_case_name'] = case.name
    ...
    if request.method == "POST":
        ...
        # Initialize default context to prevent undefined errors
        current_case.update({
            "summary": None,
            "clustering_results": {},
        ...
        try:
            ...
            # Stream/Fetch data
            # Note: For full stream we use the /stream endpoint, but for direct post we do synchronous
            # Or simpler: Redirect to a loading page that triggers the stream
            pass # The stream endpoint handles the actual fetching now
            
        except Exception as e:
            flash(f"Error initializing chain: {e}", "danger")

    # If we have data in DB, show it
    # ... (rest of render logic)
    
    # Define currency symbol for UI
    chain_name = current_case.get('chain', 'ethereum').lower()
    if 'bitcoin' in chain_name: currency = 'BTC'
    elif 'solana' in chain_name: currency = 'SOL'
    elif 'tron' in chain_name: currency = 'TRX'
    elif 'bsc' in chain_name or 'bnb' in chain_name: currency = 'BNB'
    elif 'polygon' in chain_name or 'matic' in chain_name: currency = 'MATIC'
    else: currency = 'ETH'
...
def stream_analysis(case_id):
    ...
    def generate():
        try:
            ...
            # Use Manager method instead of calling on object directly
            case_manager.add_address_to_case(case_id, address, tag="target", notes=f"Analysis started on {datetime.now()}")
            ...
            # Setup Multi-Chain Fetcher
            from multi_chain import MultiChainFetcher
            
            # Ensure we have API keys from env or global
            api_keys = {
                'ETHERSCAN_API_KEY': os.environ.get('ETHERSCAN_API_KEY') or ETHERSCAN_KEY,
                'SOLANA_API_KEY': os.environ.get('SOLANA_API_KEY'),
                'BLOCKCHAIR_API_KEY': os.environ.get('BLOCKCHAIR_API_KEY'),
                'TRON_API_KEY': os.environ.get('TRON_API_KEY', '') # Add Tron if available in env
            }
            
            fetcher = MultiChainFetcher(api_keys)
            
            # Fetch Transactions (Unified Interface)
            txs, counts = fetcher.fetch_transactions(chain_name, address)
            ...
            summary, G, source = analyze_live_eth(
                    txs, address, 
                    start_date=start_date, end_date=end_date, chain_id=chain_id, chain_name=chain_name,
                    case_id=case.id  # Pass the Database Primary Key ID
                )
...
def batch_processing():
        ...
        if addresses_input:
            ...
            try:
                ...
                for i, address in enumerate(addresses):
                    try:
                        # Fetch transactions using MultiChainFetcher if available, else eth_live
                        # For batch, we probably want to use the unified analyze_live_chain if possible
                        # but that might be slow. Let's stick to existing logic for now or update:
                        
                        # Simplified for now to fix syntax
                        from eth_live import fetch_eth_address_with_counts
                        txs, counts = fetch_eth_address_with_counts(address, ETHERSCAN_KEY, chain_id=chain_id)
                        ...
                        batch_status["results"].append({
                            ...
                            "flags": "High Risk" if summary.get('risk_score', 0) > 75 else "Clean"
                        })
    ...
    return render_template("batch_processing.html", results=results)



@app.route("/api/case/<case_id>/charts")
def get_case_charts(case_id):
    from visualization_data import get_visualization_data
    data = get_visualization_data(case_id)
    if not data: return jsonify({"error": "No data"}), 404
    return jsonify(data)

@app.route("/api/ai/analyze/<address>")
def ai_analyze_address(address):
    """Get AI-driven insights for an address"""
    try:
        from ai_engine import ai_engine
        results = ai_engine.analyze_address_behavior(address)
        return jsonify(results)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/graph/analyze/<case_id>")
def graph_analyze_case(case_id):
    """Get Graph Intelligence for a case"""
    try:
        from graph_intelligence import graph_intel
        # For simplicity, just return communities for now
        # We need a serialized format
        communities = graph_intel.detect_communities(case_id)
        # Convert {comm_id: [nodes]} to list of {id: 0, nodes: [...]}
        comm_list = [{"id": k, "nodes": v} for k, v in communities.items()]
        return jsonify({"communities": comm_list})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/graph/visualize/<case_id>")
def get_graph_visualization(case_id):
    """Get full graph for Cytoscape"""
    try:
        from graph_intelligence import graph_intel
        elements = graph_intel.get_cytoscape_elements(case_id)
        return jsonify(elements)
    except Exception as e:
         return jsonify({"error": str(e)}), 500

@app.route("/api/graph/expand/<address>")
def expand_graph_node(address):
    """Get neighbors for expansion"""
    try:
        from graph_intelligence import graph_intel
        elements = graph_intel.expand_node(address)
        return jsonify(elements)
    except Exception as e:
         return jsonify({"error": str(e)}), 500
...
@app.route("/api/contract/scan", methods=["POST"])
def scan_contract():
    """Scan smart contract source/bytecode"""
    data = request.json
    source = data.get("source_code", "")
    try:
        from contract_scanner import contract_scanner
        res = contract_scanner.scan_source_code(source)
        return jsonify(res)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/intel/lookup/<address>")
def lookup_intel(address):
    """Check Threat Intel DB"""
    try:
        from threat_intel import threat_intel
        res = threat_intel.lookup_address(address)
        return jsonify(res if res else {"found": False})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/api/tools/demix/<tx_hash>")
def detect_demixing(tx_hash):
    """Check for CoinJoin"""
    try:
        from graph_intelligence import graph_intel
        res = graph_intel.detect_coinjoin(tx_hash)
        return jsonify(res)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/tools/taint/<address>')
def run_taint_analysis(address):
    """Run recursive taint analysis on an address"""
    ...
    # We need a broader graph. For now, we'll use the case's cached graph if possible
    # Or simpler: Just fetch all txs in the case to build the graph
    all_case_txs = db.session.query(Transaction).filter_by(case_id=case_id).all()
    tx_dicts = [tx.to_dict() for tx in all_case_txs]
    
    from taint_analysis import TaintAnalyzer
    analyzer = TaintAnalyzer(tx_dicts)
    
    # Run analysis
    taint_results = analyzer.calculate_taint(address)
    
    return jsonify({
        'source': address,
        'tainted_nodes': taint_results,
        'count': len(taint_results)
    })

@app.route("/api/tools/cross_chain/<to_address>")
def check_cross_chain(to_address):
    """Check if address is a bridge"""
    try:
        from cross_chain import cross_chain_tracker
        res = cross_chain_tracker.check_bridge_interaction(to_address)
        return jsonify(res)
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/tools/generate_fir', methods=['POST'])
def generate_fir_endpoint():
    """Generate and return FIR PDF"""
    try:
        from flask import request, jsonify, session, send_from_directory
        from db_models import Case, Transaction, SessionLocal
        import os
        from datetime import datetime

        data = request.form
        investigator = data.get('investigator', 'Unknown Officer')
        department = data.get('department', 'Cyber Cell')
        case_id = session.get('active_case_id')
        ...
        try:
            ...
            total_in = sum(t.amount for t in txs if t.to_address.lower() == target_address.lower())
            total_out = sum(t.amount for t in txs if t.from_address.lower() == target_address.lower())
            ...
            summary = {
                ...
                'risk_score': 85 if 'suspect' in case.name.lower() else 45, # Mock/Placeholder if AI not run
                'entity_type': 'Suspect Wallet',
                'top_victims': [], # simplified
                'top_suspects': []
            }
            
            # Generate PDF
            from legal_report import LegalReportGenerator
            generator = LegalReportGenerator(case_id, investigator, department)
            ...
            generator.create_fir_report(summary, "Automated Blockchain Analysis", target_address, filepath)
...
@app.route("/api/predict/movement/<address>")
def predict_movement(address):
    """Predict next moves"""
    try:
        from predictive_models import predictive_analytics
        res_hop = predictive_analytics.predict_next_hop(address)
        res_time = predictive_analytics.predict_exit_time(address)
        return jsonify({"next_hop": res_hop, "timing": res_time})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route("/case/<case_id>/tracer")
def view_tracer(case_id):
    """Render the Interactive Tracer Page"""
    db = case_manager.get_db()
    case = db.query(Case).filter_by(case_id=case_id).first()
    db.close()
    if not case: return "Case not found", 404
    return render_template("tracer.html", active_case=case)
...
from monitoring_system import monitor

# Start Monitoring (threaded)
...
def get_alerts():
    ...
    # Get alerts for active case if possible, or all
    # For now, just return latest 20 unacknowledged
    db = case_manager.get_db()
    try:
        alerts = db.query(Alert).filter(Alert.is_acknowledged == False)\
