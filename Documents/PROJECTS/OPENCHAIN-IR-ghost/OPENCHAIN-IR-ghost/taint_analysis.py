...
class TaintAnalyzer:
    ...
    def calculate_taint(self, source_address: str, depth: int = 5) -> Dict[str, float]:
        """
        Calculate 'Taint Score' for all downstream addresses.
        Taint Score (0.0 - 1.0) represents the fraction of funds in a wallet 
        that originated from the source address.
        
        Algorithm: Simplified Flow-Based Taint
        1. Source has Taint=1.0
        2. When A sends X amount to B:
           Delta_Taint = (Amount_Sent / Total_Received_By_B) * Taint_A
           Taint_B += Delta_Taint
        """
        source = source_address.lower()
        taint_scores = defaultdict(float)
        taint_scores[source] = 1.0
        
        # Build a tailored subgraph for traversal (BFS)
        queue = deque([(source, 1.0, 0)]) # current_addr, incoming_taint, current_depth
        visited = set()
        
        while queue:
            current, current_taint_strength, d = queue.popleft()
            ...
            # Get all outgoing transactions from current node
            ...
            for succ in successors:
                # Calculate amount sent from CURRENT to SUCC
                edge_data = self.graph.get_edge_data(current, succ)
                amount_sent = float(edge_data.get('weight', 0))
                
                # We need to know Total Received by SUCC to calculate dilution
                # In a full node we'd query db, here we estimate from graph or assume worst case (no dilution)
                # For this implementation, we'll use a "Direct Flow" metric (Propagation of Volume)
                # Taint Transferred = Amount * Source_Taint
                
                taint_transferred = amount_sent * current_taint_strength
                
                # Update successor's taint "volume" (not percentage yet, just dirty eth amount)
                taint_scores[succ] += taint_transferred
                
                queue.append((succ, current_taint_strength, d + 1)) # Propagate strength? 
                # Actually, simplified: Taint Strength degrades? 
                # Let's use a "Dirty Volume" approach. 
                # If Source sends 10 ETH (Dirty), Receiver gets 10 ETH Dirty.
        
        # Normalize/Format results
        results = []
        for addr, score in taint_scores.items():
            ...
            results.append({
                'address': addr,
                'taint_volume': score,
                'is_mixer': addr in self.KNOWN_MIXERS.values(),
                'is_exchange': addr in self.KNOWN_CEXES.values()
            })
            
        return sorted(results, key=lambda x: x['taint_volume'], reverse=True)
